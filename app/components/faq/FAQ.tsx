                    ...
                    return (
                         <div key={index} className={`rounded-xl p-4 ${openIndex == index ? "bg-secdark" : "bg-sec"}`}>
                              <button type={"button"} className={"w-full flex font-bold"} onClick={() => toggle(index)}>
                                   {quest.question}
                                   <p
                                        className={`ml-auto inline-block transition-transform duration-120 ${openIndex == index ? "rotate-45 font-extrabold" : "rotate-0 font-normal"} `}
                                   >
                              ...
                              {openIndex == index && (
                                   <div className={"mt-3 text-sm whitespace-pre-wrap"}>{quest.answer}</div>
                              )}
                         </div>
                    )
               })}
