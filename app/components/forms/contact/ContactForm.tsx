               ...
               {!formEnd && (
                    <section className={"bg-white w-80 mx-auto my-6 p-6 rounded-xl shadow lg:w-[60%]"}>
                         ...
                         <p className={"text-center mb-3"}>
                              ...
                              <span className={"boldtext"}>ce formulaire ne vous engage en rien</span>! Vous recevrez un
                              devis fait sur-mesure par nos talentueux membres, qui discuterons avec vous des détails
                              techniques. <br />
                              Le paiement se fera en deux fois, après avoir déterminé le cahier des charges et en étant
                              sûr que
                         </p>
                         ...
                         <form className={"flex flex-col items-center gap-3"} onSubmit={handleSubmit}>
                              ...
                              <InputTextForm
                                   type={"text"}
                                   name={"service"}
                                   placeholder={"Service désiré / Sujet du message"}
                              />
                              <textarea
                                   name={"details"}
                              ...
                              <label className="flex flex-col items-center gap-3 lg:w-[90%]">
                                   <span className={"titleform"}>
                                        ... Ou téléchargez votre cahier des charges (PDF uniquements.)
                                   </span>

                                   <div className="flex flex-col items-center justify-center border border-sec rounded-sm w-full p-5 gap-4">
                                        <label htmlFor="cv-upload">
                                             Glissez votre cahier des charges ici ou cliquez pour sélectionner un
                                             fichier
                                        </label>
                              ...
                              <InputTextForm
                                   type={"text"}
                                   name={"budget"}
                                   placeholder={"Indiquez un budget"}
                                   required={true}
                              />
                              <button
                                   type={"submit"}
                                   disabled={isCooldown}
                                   className={
                                        "cursor-pointer bg-main w-full block mx-auto px-9 py-3 text-white shadow rounded-sm hover:bg-transparent hover:text-main transition-all duration-300" +
                                        "disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-main disabled:hover:text-white"
                                   }
                              >
                                   {isCooldown ? `Veuillez patienter (${secondsLeft}s)` : "Envoyer"}
                              </button>
                         </form>
                    </section>
               )}
                    ...
                    <>
                         {success && (
                              <div className={`${classDiv} items-center`}>
                                   <h2 className={"bigtitle"}>Merci pour votre message!</h2>
                                   <p className={"text-center boldtext"}>Vous avez reçu un mail de confirmation.</p>
                                   <Link
                                        href={"/"}
                                        className={
                                             "cursor-pointer bg-main text-white p-4 rounded-xl hover:scale-105 transition-all duration-300"
                                        }
                                   >
                                        Retour à l&#39;accueil
                                   </Link>
